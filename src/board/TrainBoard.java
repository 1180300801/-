...
public class TrainBoard extends JFrame {
	 ...
	 */
	public void show(TrainBoard frame) {
	 ...
	 * Create the frame.
	 */
	public TrainBoard(String S,TrainCollection fc) {
	 ...
	 * @return 信息板内容
	 */
	private String[][] getMessage(String S,TrainCollection fc){
		//信息板初始大小设定为10列，一旦列数超出该值，列数翻倍
		int num = 10;
		String[][] s = new String[num][4];
		int i = 0;
		for(TrainEntry fe:fc) {
			if(fe.getMle().getLocations().get(0).getLocationName().equals(S)) {
				...
				if(timeslot.timeSub() == 0) {
					if(timeslot.timeSubHourAndMin()>=-60|timeslot.timeSubHourAndMin()<=60) {
						...
						s[i][1] = fe.getMsre().getResources().toString();
	 ...
	 * @return 信息板内容
	 */
	private String[][] getMessage1(String S,TrainCollection fc){
		//信息板初始大小设定为10列，一旦列数超出该值，列数翻倍
		int num = 10;
		String[][] s = new String[num][4];
		int i = 0;
		for(TrainEntry fe:fc) {
			if(fe.getMle().getLocations().get(fe.getMle().getLocations().size()-1).getLocationName().equals(S)) {
				String str = new SimpleDateFormat("yyyy-MM-dd hh:mm").format(new Date());
				Timeslot timeslot = new Timeslot(str,fe.getStartAndEndTime().getEndTime());
				if(timeslot.timeSub() == 0) {
					if(timeslot.timeSubHourAndMin()>=-60|timeslot.timeSubHourAndMin()<=60) {
